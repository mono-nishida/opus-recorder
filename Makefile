...
$(LIBOPUS_DECODER): $(LIBOPUS_DECODER_SRC) $(LIBOPUS_OBJ) $(LIBSPEEXDSP_OBJ)
	npx webpack --config webpack.debug.js -d --output-library DecoderWorker --entry $(LIBOPUS_DECODER_SRC) --output-filename $(LIBOPUS_DECODER_FILENAME)
...
$(RECORDER): $(RECORDER_SRC)
	npx webpack --config webpack.debug.js -d --output-library Recorder --entry $(RECORDER_SRC) --output-filename $(RECORDER_FILENAME)

$(WAVE_WORKER): $(WAVE_WORKER_SRC)
	npx webpack -d --entry $(WAVE_WORKER_SRC) --output-filename $(WAVE_WORKER_FILENAME)
